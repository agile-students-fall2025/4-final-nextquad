...
const jwtVerifyToken = async function (jwt_payload, next) {
  ...
  const adminId = new ObjectId(jwt_payload.userid); // convert the string id to an ObjectId
  const admin = await Admin.findOne({ _id: adminId }).exec();
  if (admin) {
    // we found the admin... keep going
    next(null, admin);
  } else {
    // we didn't find the admin... fail!
    next(null, false, { message: "Admin not found" });
  }
};
