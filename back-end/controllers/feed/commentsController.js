const { formatRelativeTime } = require("../../data/feed/mockFeedData");
...
const getPostComments = async (req, res) => {
  try {
    ...
    console.log(
      `Fetching comments for post ${postId}: Found ${comments.length} comments`
    );
    ...
    res
      .status(200)
      .json({ success: true, count: withLikeFlag.length, data: withLikeFlag });
  } catch (error) {
    console.error("[getPostComments] error:", error);
    res
      .status(500)
      .json({ success: false, error: "Server error while fetching comments" });
  }
};
...
const addComment = async (req, res) => {
  try {
    ...
    console.log(
      `Creating comment for post ${postId}. Last comment ID: ${
        last?.id || "none"
      }, Next comment ID: ${nextId}`
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Server error while adding comment" });
  }
};
...
const updateComment = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Server error while updating comment" });
  }
};
...
const deleteComment = async (req, res) => {
  try {
    ...
    if (
      comment.author.userId !== req.user.userId &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
    ...
    res
      .status(500)
      .json({ success: false, error: "Server error while deleting comment" });
  }
};
